...
@Component
@Slf4j
public class ChattingWebsocketHadler extends TextWebSocketHandler {
  ...
  public void afterConnectionEstablished(WebSocketSession session) throws Exception {
    ...
    // Message 객체에 대입
		...
		// -> 로그인한 회원 (session)
		// -> websocketSession에 담겨 있음!! (인터셉터 참고)
		...
		int senderNo = sendMember.getMemberNo(); // 보낸회원 번호
		
		msg.setSenderNo(senderNo);// Message 객체에 세팅
		
		//------------------------ DB INSERT ------------------------------------
		
		//1) ChattingService 의존성 주입받기
