package edu.kh.project.common.fiter;
...
public class SignUpFilter implements Filter{
	...
	public void doFilter(
			ServletRequest request, 		// HttpServletRequest의 부모타입
		...
		// 위 작성된 것은 부모타입이라 적용이 제대로 안됨 
		// HTTP 통신이 가능한 현태로 request, reponsr 다운 캐스팅
		HttpServletRequest req = (HttpServletRequest)request;
		HttpServletResponse resp = (HttpServletResponse)response;
		
		// 세션에 로그인한 회원 정보가 있을 경우 
		...
		if(session.getAttribute("loginMember") != null) {
			...
			// 다음 필터로 이동
			// (만약 다음 필터가 없다면 Dispathcher Servlet으로 이동)
