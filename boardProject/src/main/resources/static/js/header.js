...
//-------------------------------------------------------------------------------------------------
 ...
 eventSource.addEventListener("message", e => {
  ...
  const notificationBtn 
   ...
   notificationBtn.classList.remove("fa-refular");
...
/* 알림 메시지 전송 함수 
 - 알림을 받을 특정 클라이언트의 id(memberNo 또는 memberNo를 알아낼수 있는값)

 [동작 원리]

 1) AJAX를 이용해 SseController에 요청 
 2) 연결된 클라이언트 대기명단 (emitter)에서
    클라이언트 id가 일치하는 회원을 찾아
    메세지 전달하는 send() 메서드 수행
...
notificationBtn?.addEventListener("click",()=>{
  ...
  // 알림 목록이 보이고 있을 경우
  if(notificationBtn.classList.contains("notification-show")){

    // 안보이게 하기 
    notificationList.classList.remove("norification-show");
