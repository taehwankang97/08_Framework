...
// Controller : 어떤 요청을 받고, 어떻게 응답할지 제어하는 역활
// - 어떤 요청을 처리할지 주소 매핑 
...
public class ForwardTestController {
	...
	// Constroller 메서드 작성 방법 

	// 1)접근 제한자는 무조건 public
	
	// 2) 반환형은 대부분 String 
	//  (modelAndView 또는 Ajax 사용시 달라질수 있음)
	// 왜 String 인가 ? 
	// -> Controller 메서드에서 반환되는 문자열이
	// -> forward할 html파일의 경로가 되기때문
	
	
	// 3) 메서드 명은 의미있게 마음대로 작성
	
	// 4) !! 매개변수는 필요한 만큼 마음대로 !!
	//  Arguments Resolver 참조 
	
	// 5) 매핑할 요청 주소를 @RequsetMapping 등을 이용해 작성
